...
export class ReviewEntity1648410971215 implements MigrationInterface {
    name = 'ReviewEntity1648410971215'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`review\` (\`id\` int NOT NULL AUTO_INCREMENT, \`rating\` enum ('1', '2', '3', '4', '5') NOT NULL DEFAULT '5', \`comment\` varchar(1000) NOT NULL, \`createdDate\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`isApproved\` tinyint NOT NULL DEFAULT 0, \`carId\` int NOT NULL, \`userId\` varchar(255) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`car\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`model\` int NOT NULL, \`number\` int NOT NULL, \`quantity\` int NOT NULL, \`price\` int NOT NULL, \`warranty\` int NOT NULL, \`distributorId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
