...
function CarDetailPage() {
    ...
    const { data: car } = useSWR<ICar>(`/api/cars/${carId}`, fetcher)
                ...
                <div className="mt-20">
                    <div className="flex justify-center mobile:flex-col mobile:mt-4 mobile:p-3">
                        ...
                        <div
                            ...
                            <ReviewsButton carId={carId} averageRating={car.average_rating ? car.average_rating : 0}
                                           reviewCount={car.review_count ? car.review_count : 0} />
                            ...
                            <div className="flex flex-col place-self-start">
                                ...
                                <p className="mt-2 text-2xl">
                                    Warranty: <b>{car.warranty} year{car.warranty === 1 ? "" : "s"}</b>
                                </p>
                                <p className="mt-2 text-2xl">
                            ...
                            <div className="flex flex-row justify-start items-baseline gap-x-8 mt-5">
                                {
                                    car.quantity ?
                                        <TextField
                                            focused
                                            value={amount}
                                            onChange={(event) => setAmount(Math.max(Math.min(parseInt(event.target.value), car.quantity), 1))}
                                            label="Quantity"
                                            type="number"
                                            InputLabelProps={{
                                                shrink: true,
                                ...
                                {
                                    !car.quantity ?
                                    <Button endIcon={<RemoveShoppingCart />}
                                            className="text-white bg-[#000] p-3 mr-5"
                                            sx={{
                                                minWidth: 250,
                                                maxHeight: 35,
